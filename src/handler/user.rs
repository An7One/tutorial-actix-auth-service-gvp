...
use crate::{
    ...
    error::AppError,
    model::user::{NewUser, UpdateProfile, User},
};

#[instrument(skip(crypto_service, repository,  user))]
    ...
    match result{
        ...
        Err(error) => {
            let pg_error: &PgDatabaseError = 
                error.root_cause().downcast_ref::<PgDatabaseError>()
                .ok_or_else(||{
                    debug!("Error with creating the user. {:?}", error);
                    AppError::INTERNAL_ERROR
                })?;
                
            let error = match (pg_error.code(), pg_error.column()) {
                (db::UNIQUE_VIOLATION_CODE, Some("email")) => {
                    AppError::INVALID_INPUT.message("Email address already exists.".to_string())
                },
                (db::UNIQUE_VIOLATION_CODE, Some("username")) => {
